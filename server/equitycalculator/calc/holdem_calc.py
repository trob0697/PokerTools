import time
from .holdem_argparser import *
from .holdem_functions import *

def main():
    hole_cards, num, exact, board, file_name = parse_args()
    run(hole_cards, num, exact, board, file_name, True)

def calculate(board, exact, num, input_file, hole_cards, verbose):
    args = LibArgs(board, exact, num, input_file, hole_cards)
    hole_cards, n, e, board, filename = parse_lib_args(args)
    ...
    if file_name:
        ...
        for line in input_file:
            if line is not None and len(line.strip()) == 0:
                continue
            hole_cards, board = parse_file_args(line)
...
def run_simulation(hole_cards, num, exact, given_board, deck, verbose):
    ...
    if (None, None) in hole_cards:
        ...
        find_winner(generate_boards, deck, hole_cards, num,
                                     ...
                                     result_histograms)
    if verbose:
        print_results(hole_cards, winner_list,
                                       result_histograms)
